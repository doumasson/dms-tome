...
export function crToXp(cr) {
  ...
  return XP[String(cr)] ?? XP[Number(cr)] ?? 0;
}

// XP thresholds per character level for milestone reference
const THRESHOLDS = [
  [0,0,0,0], // placeholder
...
export function getXpThresholds(level) {
  const lv = Math.max(1, Math.min(20, parseInt(level) || 1));
  const [easy, med, hard, deadly] = THRESHOLDS[lv];
  return { easy, medium: med, hard, deadly };
}
