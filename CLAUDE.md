...
- Dice roller, loot generator (DM tool), notes tab
...
- **Production TDZ fix** — `const inCombat` in `GameLayout.jsx` was declared after `useEffect` calls that referenced it in dependency arrays; Rollup's minifier exposed the TDZ as "Cannot access 'w' before initialization" on every page load; fixed by moving the declaration above the hooks that depend on it. General rule: always declare `const`/`let` values before any hooks that reference them in dependency arrays.
